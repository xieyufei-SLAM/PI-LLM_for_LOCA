...
def compute_losses(model, physics, batch, device, w):
    ...
    out = model(history, suffix, prompts)
    pred = out['pred']
    mse = nn.functional.mse_loss(pred, target)
    loss_sace = out['loss_sace']
    loss_s2t = out['loss_s2t']

    nc = min(getattr(physics, 'n_colloc', pred.size(0)), history.size(0))
    colloc = batch['colloc'][:nc].to(device).clone().requires_grad_(True)
    hist_c, suf_c = inject_colloc(history[:nc], suffix[:nc], colloc)
    pred_c = model(hist_c, suf_c, prompts[:nc])['pred']
    loss_phy, phy_parts = physics(colloc, pred_c)
...
def main():
    ap = argparse.ArgumentParser('PI-LLM')
    ...
    print(f"{seg}: train={len(tr)} val={len(va)} | device={device}")
    ...
    # torch 2.0.1 的 CUDA triu 不支持 bf16(Llama causal mask 会用到)，故用 fp16
